...
?>
<div class="">
    <h3><i class="fa fa-arrow-circle-up"></i> <?= $page_title; ?><a href="<?= site_url('check_in/add') ?>" class="btn btn-primary pull-right" style="margin-right:5px;"><i class="fa fa-plus"></i> <?= lang('Add Inbound'); ?></a></h3>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        var table = $('#TTable').DataTable({
            ...
            "serverSide": true,
            'ajax': {
                url: '<?= site_url('check_in/get_list/' . $v); ?>',
                type: 'POST',
                "data": function(d) {
    ...
    function detail(id) {
        $.get('<?= base_url('check_in/view/') ?>' + id, function(data) {
            $('#myModal').html(data);
            $('#myModal').modal('show');
        });
    }
